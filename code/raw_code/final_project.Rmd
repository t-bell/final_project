...
```{r setup, include=FALSE, catch = F}
## install packages if necessary
list.of.packages <- c("janitor", "ggplot2", "dplyr", "lubridate", "zoo", "forcats", "ggthemes", "knitr", "rmarkdown", "nanair", "tidyr")
...
In this graph we can see that a respondent's income* does not neccesarily affect the amount of time spent with their respective child, other than the obvious outliers within each tax bracket. Although you can see a slight rise, or peak, after the 12th tax bracket!
...
```
With the Marital Status Key* in mind \*(see below)\*, we can see that married respondents with the spouse presnt on average spend more time with their children than any of the other categories of respondents. 
...
idk bro, i'm gettting frustrated because whet T.T
...
went to here [https://github.com/halhen/viz-pub/tree/master/pastime-income](https://github.com/halhen/viz-pub/tree/master/pastime-income) for help. that's a lot of code I never seen lol. gonna keep searching to help understand!
```{r save-plot}
## save the plot above
ggsave(filename = "activity_by_income.pdf", plot = last_plot())
