...
list.of.packages <- c("janitor", "ggplot2", "dplyr", "lubridate", "zoo", "forcats", "ggthemes", "knitr", "rmarkdown", "tidyr", "devtools", "ggpubr")
...
# Plot the graph!  
    ggplot(aes(x = TEAGE, y = AVGMINS)) +
        geom_bar(stat = "identity", aes(color = factor(ACTIVITY)))+
        coord_flip()
```         
We were asked to plot the respondent's age against the average amount of time for each activity provided! 
...
activity_by_income <-atus.long %>% 
  ...
  ggplot(aes(x = ACTIVITY,y = HEFAMINC, fill = "AVGMIN_WRK")) +
