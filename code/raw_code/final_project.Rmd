...
adults_atLeast_one_kid <- atus.all %>%
 ...
 ggplot(adults_atLeast_one_kid, aes(x = TEAGE, y = CHILDCARE, col = PEMARITL)) +
      geom_point() +
...
\** Note that this data only includes respondents with at least one child within the household that they take care of. And this variable will stay constant for the next three graphs! CHECK MARITAL STATUS PLOT FOR KEY!*\*
...
activity_by_income <-atus.long %>% 
  ...
  ggplot(aes(x = ACTIVITY,y = HEFAMINC, fill = "variable")) +
