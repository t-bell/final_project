...
atus.all <- atus.all %>% 
  mutate(CHILDCARE = sum(atus.all$t030101:atus.all$t030112) %>%
           glimpse(CHILDCARE))
```
...
atus.all %>% 
    group_by(TESEX) %>% 
   summarise(men_avg_childcare <- if('atus.all$TESEX' == 1) {mean(CHILDCARE)} %>%
             women_avg_childcare = if('atus.all$TESEX' == 2) {mean(CHILDCARE)} %>%
   arrange(men_avg_childcare, women_avg_childcare))
```
...
## do younger people spend more time than older people.
filter(atus.all$)
