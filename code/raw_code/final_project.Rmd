...
adults_atLeast_one_kid <- atus.all %>%
  select(CHILDCARE, TEAGE, TRYHHCHILD, HEFAMINC, TRCHILDNUM) %>%
       filter(TRCHILDNUM >  0)
  ggplot(adults_atLeast_one_kid, aes(x = TEAGE, y = CHILDCARE)) +
      geom_point() 
## do richer people spend more time with children than poor
income_factor <- atus.all %>%
  mutate(all_income_by_age = TEAGE + HEFAMINC) %>%
  select(CHILDCARE, TEAGE, TRYHHCHILD, HEFAMINC) %>%
  ggplot(atus.all, aes(x = HEFAMINC, y = CHILDCARE)) +
      geom_point() +
