...
```
##take this away, filter once done. 
...
names(atus.all) %>%
  select(atus.all, CHILDCARE, TEAGE, TRYHHCHILD, HEFAMINC) %>%
       filter(atus.all, TEAGE, TRYHHCHILD) %>%
  ggplot(atus.all, aes(x = TEAGE, y = CHILDCARE)) +
      geom_point()
## do richer people spend more time with children than poor
names(atus.all) %>%
  select(atus.all, CHILDCARE, TEAGE, TRYHHCHILD, HEFAMINC) %>%
 income_factor <- mutate(atus.all, all_income_by_age = TEAGE + HEFAMINC) %>%
  ggplot(atus.all, aes(x = income_factor, y = CHILDCARE)) +
      geom_point()
